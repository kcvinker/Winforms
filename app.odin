...
test_proc :: proc(s : ^Control, e : ^MouseEventArgs) {          
    ...
    //f.set_form_state(frm, f.FormState.minimized)
    //f.set_text(&frm, "Just changed")
    // n := f.current_time(f.TimeMode.milli_sec)
    // ptf("time in miro %d\n", n)
    print("----------------------------------------") 
}

btn_clk :: proc(s : ^Control, e : ^ui.MoveEventArgs) {  
    print("form moved to  this loc")
    ptf("x - %d, y - %d\n", e.x, e.y)
    //ui.print_rect(e.form_rect^)
    //f.msg_box(f.get_combo_selected_item(&mb))
  // f.set_combo_style(&mb, f.DropDownStyle.lb_combo)
...
cbMleave :: proc(s : ^Control, e : ^EventArgs) {
    //print("calendar value - ", ui.date_string(c.value))
    //print("cal view - ", c.view_mode)
}
