...
@private lbox_ctor :: proc(p : ^Form, x, y, w, h : int) -> ListBox {
	...
	lbx._beforeCreation = cast(CreateDelegate) lbx_before_creation
	lbx._afterCreation = cast(CreateDelegate) lbx_after_creation
    lbx._dummyIndex = -1
    return lbx
}

// Create new listbox type.
newListBox :: proc{lbox_ctor1, lbox_ctor2, lbox_ctor3}
...
@private lbx_wnd_proc :: proc "std" (hw : HWND, msg : u32, wp : WPARAM, lp : LPARAM,
    ...
    switch msg {
        ...
        case WM_LBUTTONDOWN:
            ...
            if lbx.onLeftMouseDown != nil {
                mea := new_mouse_event_args(msg, wp, lp)
                lbx.onLeftMouseDown(lbx, &mea)
                return 0
            }
        case WM_LBUTTONUP :
            if lbx.onLeftMouseUp != nil {
                mea := new_mouse_event_args(msg, wp, lp)
                lbx.onLeftMouseUp(lbx, &mea)
            }
