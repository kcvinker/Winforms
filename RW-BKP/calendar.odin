...
@private calendar_ctor :: proc(p : ^Form, x, y : int) -> Calendar {
    ...
    c._beforeCreation = cast(CreateDelegate) cal_before_creation
	c._afterCreation = cast(CreateDelegate) cal_after_creation

    return c
}

// Create a new Calendar control.
newCalendar :: proc{new_cal1, new_cal2}
   ...
   //display_msg(msg)
    switch msg {
        ...
        case WM_MOUSELEAVE :
            ...
            if cal.onLeftMouseDown != nil {
                mea := new_mouse_event_args(msg, wp, lp)
                cal.onLeftMouseDown(cal, &mea)
                return 0
            }
        ...
        case WM_LBUTTONUP :
            if cal.onLeftMouseUp != nil {
                mea := new_mouse_event_args(msg, wp, lp)
                cal.onLeftMouseUp(cal, &mea)
            }
