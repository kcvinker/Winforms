...
newButton :: proc{new_button1, new_button2, new_button3, new_button4}
...
@private buttonCtor :: proc(p : ^Form, txt : string, x, y, w, h : int) -> Button
{
	...
	b._beforeCreation = cast(CreateDelegate) btn_before_creation
	b._afterCreation = cast(CreateDelegate) btn_after_creation
	return b
}
									...
									sc_id : UINT_PTR, ref_data : DWORD_PTR) -> LRESULT
{
	...
	switch msg
	{
		...
		case WM_LBUTTONDOWN:
			...
			if btn.onLeftMouseDown != nil
			{
				mea := new_mouse_event_args(msg, wp, lp)
				btn.onLeftMouseDown(btn, &mea)
			}
		case WM_RBUTTONDOWN:
		...
		case WM_LBUTTONUP :
			if btn.onLeftMouseUp != nil
			{
				mea := new_mouse_event_args(msg, wp, lp)
				btn.onLeftMouseUp(btn, &mea)
			}
