...
new_listviewitem :: proc {
	listview_item_constructor1,
	listview_item_constructor2,
	listview_item_constructor3,
}
...
new_listviewcolumn_array :: proc(name_and_width: ..any) -> [dynamic]^ListViewColumn {
	...
	for item in name_and_width {
		if value, is_str := item.(string);
		   is_str {append(&colnames, value)} else if value, is_int := item.(int); is_int {append(&colwidths, value)}
	}
...
listview_add_item :: proc(lv: ^ListView, lvi: ^ListViewItem) {
	...
	if lvi.imageIndex > -1 do item.mask |= LVIF_IMAGE
	item.state = 0
	item.stateMask = 0
	item.iItem = lv._index
	item.iSubItem = 0
	lvi.index = int(lv._index)
	item.iImage = cast(i32)lvi.imageIndex
	item.pszText = to_wstring(lvi.text)
	item.cchTextMax = i32(len(lvi.text))
	item.lParam = dir_cast(lvi, LPARAM)
...
@(private)
lv_constructor :: proc(f: ^Form, x, y, w, h: int) -> ^ListView {
	if lvcount == 0 {
...
@(private)
lv_constructor1 :: proc(parent: ^Form) -> ^ListView {
...
@(private)
lv_constructor2 :: proc(parent: ^Form, x, y: int) -> ^ListView {
...
@(private)
lv_constructor3 :: proc(parent: ^Form, x, y, w, h: int) -> ^ListView {
...
@(private)
lv_constructor4 :: proc(parent: ^Form, x, y, w, h: int, colnames: ..string) -> ^ListView {
...
@(private)
lv_constructor6 :: proc(parent: ^Form, x, y, w, h: int, coldata: ..any) -> ^ListView {
	this := lv_constructor(parent, x, y, w, h)
	if parent.createChilds do create_control(this)
	colnames: [dynamic]string
	colwidths: [dynamic]int
	...
	// Extracting column names and widths
	for item in coldata {
		if value, is_str := item.(string); is_str {append(&colnames, value)} else // LEAK
		if value, is_int := item.(int); is_int {append(&colwidths, value)} 	// LEAK
	}
	if len(colnames) == len(colwidths) { 	// If they are same, we can proceed
		for col, i in colnames {
...
@(private)
lv_constructor7 :: proc(parent: ^Form, x, y, w, h: int, cols: []^ListViewColumn) -> ^ListView {
	this := lv_constructor(parent, x, y, w, h)
	if parent.createChilds do create_control(this)
...
@(private)
lv_col_constructor1 :: proc(txt: string, width: int) -> ^ListViewColumn {
...
@(private)
lv_col_constructor2 :: proc(txt: string) -> ^ListViewColumn {
	lvc := new(ListViewColumn)
	lvc.text = txt
	lvc.width = 100
	lvc.hasImage = false
...
@(private)
lv_col_constructor3 :: proc(
	txt: string,
	width: int,
	col_align: ColumnAlignment = .Left,
	hdr_align: HeaderAlignment = .Center,
) -> ^ListViewColumn {
	lvc := new(ListViewColumn)
	lvc.text = txt
	lvc.width = width
...
@(private)
lv_item_constructor :: proc(txt: string, bgc: uint, fgc: uint, img: int = -1) -> ^ListViewItem {
...
@(private)
listview_item_constructor1 :: proc(txt: string) -> ^ListViewItem {
	lvi := lv_item_constructor(txt, white, black)
	return lvi
}

@(private)
listview_item_constructor2 :: proc(txt: string, bk_clr, fr_clr: uint) -> ^ListViewItem {
	lvi := lv_item_constructor(txt, bk_clr, fr_clr)
	return lvi
}

@(private)
listview_item_constructor3 :: proc(txt: string, img: int) -> ^ListViewItem {
...
@(private)
lv_get_header :: proc(lvh: HWND) -> HWND {return(
		cast(HWND)cast(UINT_PTR)SendMessage(lvh, LVM_GETHEADER, 0, 0) \
	)}
...
@(private)
set_coloumn_autosize :: proc(lv: ^ListView, colname: string) -> int {
...
lv_after_creation :: proc(lv: ^ListView) {
	...
	if len(lv._lvcList) > 0 {
		...
		for &col in lv._lvcList {
			res = i32(
				SendMessage(lv.handle, LVM_INSERTCOLUMNW, WPARAM(res), dir_cast(&col, LPARAM)),
			)
			res += 1
		}
...
@(private)
listview_property_setter :: proc(this: ^ListView, prop: ListViewProps, value: $T) {
	switch prop {
	case .Item_Alignment:
		break
	...
	case .Multi_Selection:
		break
	case .Has_Check_Boxes:
		break
	case .Full_Row_Select:
		break
	case .Show_Grid_Lines:
		break
	case .One_Click_Activate:
		break
	case .No_Track_Select:
		break
	case .Edit_Label:
		break
	case .No_Header:
	...
	case .Header_Height:
		break
	case .Header_Clickable:
		break
	}
}

@(private)
lv_finalize :: proc(this: ^ListView, scid: UINT_PTR) {
...
lv_wnd_proc :: proc "stdcall" (
	...
	case WM_NOTIFY:
		...
		switch nmh.code {
		case NM_CUSTOMDRAW:
			// Let's draw header back & fore colors
