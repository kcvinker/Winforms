...
Control :: struct {
	...
	_mDownHappened,
	_mRDownHappened:                                                                                          bool,
	_SizeIncr:                                                                                                                SizeIncrement,
	_clsName:                                                                                                                 wstring,
	_drawFlag:                                                                                                                uint,
	_fp_beforeCreation,
	_fp_afterCreation:                                                                                    CreateDelegate,
...
@(private)
ctl_send_msg :: #force_inline proc(hw: HWND, msg: UINT, wp: $T, lp: $U) -> LRESULT {
...
//=================================================================Private Functions==========================
@(private)
control_cast :: proc($T: typeid, refd: DWORD_PTR) -> ^T {
...
@(private)
map_parent_points :: proc(this: ^Control) -> RECT {
	rc: RECT
	firstHwnd: HWND
	if this._isCreated {
		...
		rc = RECT {
			i32(this.xpos),
			i32(this.ypos),
			i32(this.xpos + this.width),
			i32(this.ypos + this.height),
		}
	}
...
@(private)
ctrl_set_font :: proc(this: ^Control, value: $T) {
...
@(private)
control_property_setter :: proc(this: ^Control, prop: CommonProps, value: $T) {
	switch prop {
	case .Back_Color:
		set_back_color1(this, uint(value))
	case .Font:
		ctrl_set_font(this, value)
	case .Fore_Color:
		set_fore_color1(this, uint(value))
	case .Enabled:
		control_enable(this, bool(value))
	case .Height:
		control_set_size(this, this.width, int(value))
	case .Text:
		control_set_text(this, tostring(value))
	case .Visible:
		control_visibile(this, bool(value))
	case .Width:
		control_set_size(this, int(value), this.height)
	case .Xpos:
		control_set_position(this, int(value), this.ypos)
	case .Ypos:
		control_set_position(this, this.xpos, int(value))
	}
}
