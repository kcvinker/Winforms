...
@(private)
gb_ctor :: proc(p: ^Form, txt: string, x, y, w, h: int, gStyle: GroupBoxStyle) -> ^GroupBox {
	...
	gb._gbStyle = gStyle
	gb.parent = p
	gb._wtext = new_widestring(txt)
	gb.xpos = x
	gb.ypos = y
	gb.text = txt
	gb.width = w
	gb.height = h
	gb.backColor = p.backColor
	gb.foreColor = p.foreColor
	gb._clsName = &btnclass[0]
	gb._fp_beforeCreation = cast(CreateDelegate)gb_before_creation
	gb._fp_afterCreation = cast(CreateDelegate)gb_after_creation
	gb._style = gbstyleFlag
	gb._exStyle = gbexstyle // WS_EX_TRANSPARENT | WS_EX_RIGHTSCROLLBAR
...
@(private)
gb_ctor1 :: proc(parent: ^Form) -> ^GroupBox {
...
@(private)
gb_before_creation :: proc(this: ^GroupBox) {
...
@(private)
gb_after_creation :: proc(this: ^GroupBox) {
...
@(private)
resetGdiObjects :: proc(this: ^GroupBox, brpn: b64) {
	if brpn {
...
@(private)
gbx_property_setter :: proc(this: ^GroupBox, prop: GroupBoxProps, value: $T) {
	switch prop {
	case .Back_Color:
...
@(private)
gb_finalize :: proc(this: ^GroupBox) {
...
gb_wnd_proc :: proc "stdcall" (
	...
	case WM_MOUSEMOVE:
		// Mouse Enter & Mouse Move is happening here.
