...
rgb_clamp :: proc(value: uint) -> uint { return clamp(value, 0, 255) }
...
change_color_uint :: proc(clr : uint, adj : f64) -> RgbColor {
   ...
   rc.red = rgb_clamp(cast(uint) (f64(rc.red) * adj))
   rc.green = rgb_clamp(cast(uint) (f64( rc.green) * adj))
   rc.blue = rgb_clamp(cast(uint) (f64( rc.blue) * adj))
   return rc
}
...
change_color_get_ref :: proc(rgbc : RgbColor, adj : f64) -> ColorRef {
   rc := rgbc
   rc.red = rgb_clamp(cast(uint) (f64(rc.red) * adj))
   rc.green = rgb_clamp(cast(uint) (f64( rc.green) * adj))
   rc.blue = rgb_clamp(cast(uint) (f64( rc.blue) * adj))
