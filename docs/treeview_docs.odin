...
// Constructor
newTreeView :: proc(parent : ^Form) -> TreeView
newTreeView :: proc(parent : ^Form, x, y, w, h : int) -> TreeView
...
treeview_set_node_color :: proc(tv : ^TreeView, node : ^TreeNode)
    ...
    // just set the backColor & foreColor properties.
...
TreeNode :: struct {
    ...
    foreColor : UINT,             // Text color of node.
    backColor : UINT,             // Back color of the text area of node.
}
...
MakeWindow :: proc() {
    ...
    tv.backColor = 0xC6C6FF
    tv.foreColor = 0x008000
    tv.line_color = red
    create_treeview(&tv)
